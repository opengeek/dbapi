...
class DBAPI {
    ...
    function __construct(&$modx, $options = array()) {
        ...
        if (empty($host) || empty($dbase) || empty($uid) || empty($pwd)) {
            $this->xpdo= & $modx;
        } elseif (!empty($host)) {
            if (empty($dbtype)) {
                $dbtype= 'mysql';
        ...
        if (is_object($this->xpdo) && $this->xpdo instanceof xPDO) {
            $this->config['host']= $this->xpdo->getOption('host');
            $this->config['dbase']= $this->xpdo->getOption('dbname');
            $this->config['user']= $this->xpdo->getOption('username');
            $this->config['pass']= $this->xpdo->getOption('password');
            $this->config['table_prefix']= $this->xpdo->getOption('table_prefix');
        }
    }
    ...
    /**
     * Connect to database. [DO NOT USE]
     *
     * Do not call this method directly! If you need a connection to another db,
     * load another instance via modX::getService(). This implementation of the DBAPI
     * uses xPDO which creates a connection (PDO object) on demand.
     * 
     * @access public
     */
    ...
    /**
     * Disconnect from the DB. [DO NOT USE]
     *
     * Do not call this method directly! The connection will be closed when the PDO
    ...
    /**
     * Mainly for internal use. Developers should use select, update, insert, delete where possible.
     *
     * @access public
     */
